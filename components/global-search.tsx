...
import { Search, BookOpen, Video, BarChart3, User, Clock, ArrowRight, X, Hash } from 'lucide-react'
import { useAppStore } from '@/store/use-app-store'
import { courses, liveClasses } from '@/data/mock-data'
import { useAuthStore } from '@/store/use-auth-store'
import { cn } from '@/lib/utils'

type ResultItem = {
    id: string
    type: 'course' | 'live' | 'page' | 'recent'
    title: string
    subtitle?: string
    href: string
...
export function GlobalSearch() {
    ...
    const courseResults: ResultItem[] = courses
        .filter(c => searchQuery.length > 1 && (c.title.toLowerCase().includes(searchQuery.toLowerCase()) || c.category.toLowerCase().includes(searchQuery.toLowerCase())))
        .slice(0, 4)
        .map(c => ({
    ...
    const liveResults: ResultItem[] = liveClasses
        .filter(lc => searchQuery.length > 1 && lc.title.toLowerCase().includes(searchQuery.toLowerCase()) && lc.status !== 'COMPLETED')
        .slice(0, 2)
        .map(lc => ({
    ...
    const recentItems: ResultItem[] = !searchQuery ? recentSearches.slice(0, 3).map(s => ({
        id: `r-${s}`, type: 'recent' as const, title: s,
        href: `/student/courses?q=${encodeURIComponent(s)}`,
        icon: <Clock size={15} />,
    })) : []

    const results: { group: string; items: ResultItem[] }[] = searchQuery
        ? [
        ...
        : [
            ...
            ...(recentItems.length ? [{ group: 'Recent Searches', items: recentItems }] : []),
        ]

    const allItems = results.flatMap(r => r.items)

    useEffect(() => {
        if (!isSearchOpen) { setSelectedIdx(0); return }
            ...
            <div className="fixed inset-0 z-[200] flex items-start justify-center pt-[12vh] px-4">
                ...
                >
                    ...
                    <div className="flex items-center gap-3 px-4 py-3.5 border-b border-border dark:border-dark-border">
                        ...
                        />
                        {searchQuery && (
                            <button onClick={() => setSearchQuery('')} className="text-text-faint hover:text-text-muted transition-colors">
                                <X size={15} />
                            </button>
                        )}
                        <kbd className="hidden sm:flex items-center gap-0.5 text-xs bg-background dark:bg-dark-bg border border-border dark:border-dark-border text-text-faint px-1.5 py-0.5 rounded font-mono">ESC</kbd>
                    </div>
                    ...
                    <div className="max-h-[400px] overflow-y-auto py-2">
                        ...
                        {results.map(group => (
                            <div key={group.group} className="mb-1">
                                <p className="px-4 py-1.5 text-[11px] font-semibold text-text-faint uppercase tracking-wider">{group.group}</p>
                                        ...
                                        >
                                            <span className={cn('w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0',
                                                item.type === 'live' ? 'bg-accent/10 text-accent' :
                                                    item.type === 'course' ? 'bg-primary-tint text-primary dark:bg-dark-surface2 dark:text-blue-300' :
                    ...
                    <div className="border-t border-border dark:border-dark-border px-4 py-2 flex items-center gap-4 text-xs text-text-faint">
                        <span className="flex items-center gap-1"><kbd className="bg-background dark:bg-dark-bg border border-border dark:border-dark-border px-1 py-0.5 rounded font-mono">↑↓</kbd> navigate</span>
                        <span className="flex items-center gap-1"><kbd className="bg-background dark:bg-dark-bg border border-border dark:border-dark-border px-1 py-0.5 rounded font-mono">↵</kbd> open</span>
                        <span className="flex items-center gap-1"><kbd className="bg-background dark:bg-dark-bg border border-border dark:border-dark-border px-1 py-0.5 rounded font-mono">ESC</kbd> close</span>
                    </div>
                </motion.div>
            </div>
        </AnimatePresence>
